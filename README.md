...
Fresh structural helpers make it easy to manipulate immutable tuples without losing data: the `structs` module clones, concatenates, and surgically replaces tuple slots, while the `serde` module turns any ApexLang value into JSON (compact + pretty), YAML-compatible JSON, XML, or byte tuples (and back again). The dedicated [`docs/SERDE_PLAYBOOK.md`](docs/SERDE_PLAYBOOK.md) walks through each conversion strategy, object encoding rules, and how the serialization helpers compose with the structural copy utilities.
...
| `structs` | Structural copy helpers: `structs.copy` clones any value, `structs.clone_tuple` deep-clones tuple entries, `structs.copy_replace` returns a tuple with one slot replaced (without mutating the original), and `structs.tuple_concat` builds larger tuple records out of smaller pieces. |
| `serde` | Serialization bridge covering JSON/YAML-compatible text, XML, and byte tuples via `serde.to_json`, `serde.pretty_json`, `serde.from_json`, `serde.to_yaml`, `serde.from_yaml`, `serde.to_xml`, `serde.from_xml`, `serde.to_bytes`, and `serde.from_bytes`. See [`docs/SERDE_PLAYBOOK.md`](docs/SERDE_PLAYBOOK.md) for examples. |

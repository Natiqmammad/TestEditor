...
| `serde` | Serialization bridge covering JSON/YAML-compatible text, XML, CSV, and byte tuples via `serde.to_json`, `serde.pretty_json`, `serde.from_json`, `serde.to_yaml`, `serde.from_yaml`, `serde.to_xml`, `serde.from_xml`, `serde.to_csv`, `serde.from_csv`, `serde.to_bytes`, and `serde.from_bytes`. See [`docs/SERDE_PLAYBOOK.md`](docs/SERDE_PLAYBOOK.md) for examples. |

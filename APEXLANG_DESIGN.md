...
- **Ordinary & Decimal Fractions (`fractions`)**: Fraction arithmetic (`fraction_reduce`, `fraction_add/subtract/multiply/divide`), mediant/Farey helpers, terminating/repeating diagnostics (`fraction_is_terminating`, `fraction_period_length`), greedy Egyptian decompositions, decimal bridges (`fraction_to_decimal`, `decimal_to_fraction`), tuple extractors (`fraction_numerator`, `fraction_denominator`), rational comparisons (`fraction_is_reduced`, `fraction_compare`), mixed-number transforms (`fraction_to_mixed`, `fraction_from_mixed`), decimal-expansion analyzers (`fraction_decimal_parts`), continued-fraction tooling (`fraction_continued_terms`, `fraction_from_continued`), and bounded-denominator approximations (`fraction_limit_denominator`).
...
- **`mem`**: Byte-addressable buffers (`alloc_bytes`, `pointer_offset`, `read_byte`, `write_byte`, `memset`, `memcpy`), block operations (`write_block`, `read_block`, `checksum`, `find_byte`, `compare`), region mutators (`swap_ranges`, `reverse_block`, `fill_pattern`, `count_byte`), global smart pointers (`smart_pointer_new/get/set`), tuple accessors (`tuple_get`), and bitwise intrinsics (`binary_and/or/xor/not`, `binary_shift_left/right`, `binary_rotate_left/right`, `bit_test/set/clear/toggle`, `bit_count`). These helpers mirror the low-level layout controls described in §6 and make it easy to script buffer manipulations inside the interpreter.
...
- **`async`**: `async.spawn(kind, payload)` launches background jobs (`sum`, `factorial`, `prime_count`, `fibonacci`, `sleep_ms`) backed by real threads, while `async.join`, `pending`, `yield_now`, `sleep_ms`, and mailbox helpers (`mailbox_create/send/recv/try_recv/drain/len/recv_timeout/is_closed/close`) expose the worker-pool status and enable message passing in ApexLang code.
- **`fs`/`os`**: Text/binary IO (`read_text`, `write_text`, `append_text`, `read_bytes`, `write_bytes`), existence checks, copy/rename/delete helpers, directory listings and recursive walks (`read_tree`), recursive `mkdir_all`, metadata probes, file touching, temp-file creation, plus OS shims (`os.cwd`, `os.temp_dir`, `os.env_var`, `os.pointer_width`, `os.pid`, `os.args`) make it possible to stitch math-heavy routines into actual host workflows.
- **`proc`**: `proc.run(cmd, args...)` shells out to the host and returns `(exit_code, stdout, stderr)` tuples that can be deconstructed via `mem.tuple_get`, `proc.which` resolves binaries via the active `PATH`, `proc.args` captures the program arguments, `proc.env_get/env_set/env_list` expose host environment data, and `proc.cwd/set_cwd` let ApexLang code navigate directories mid-execution.
- **`net`**: Hostname resolution (`resolve_host`), IPv4 validation (`parse_ipv4`), CIDR mask generation (`subnet_mask`), subnet math (`ipv4_network`, `ipv4_broadcast`), containment tests (`cidr_contains`), private/reserved/loopback detection (`is_private_ipv4`, `is_loopback`), integer conversions (`ipv4_to_int`, `int_to_ipv4`), and address-class helpers (`ipv4_class`).

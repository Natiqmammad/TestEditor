...
use crate::ast::Type;
use crate::ast::{
    BinaryOperator, Expression, Function, Item, Literal, Pattern, Program, Statement,
};
use anyhow::Result;
use std::fmt::Write;
...
impl LLVMCodeGenerator {
    ...
    fn generate_statement(
        ...
        match stmt {
            Statement::Expression(expr) => self.generate_expression(expr, params),
            Statement::VariableDeclaration {
                name,
                type_annotation,
                value,
                            ...
                            if arms.len() > 1 {
                                if let Some(second_arm) = arms.get(1) {
                                    match &second_arm.pattern {
                                        Pattern::Wildcard => {
                                            output
                                                .push_str("  ; Wildcard pattern - default case\n");
                                            let body_code =
                                                self.generate_expression(&second_arm.body, params)?;
                                            output.push_str(&format!("  {}\n", body_code));
                                            output.push_str("  ret void\n");
                                        }
                                        Pattern::Literal(Literal::Integer(i)) => {
                                            ...
                                            output.push_str(
                                                "  br i1 %cmp1, label %match1, label %next1\n",
                                            );
                                            output.push_str("\n");
            ...
            _ => Ok("  ; TODO: Implement statement".to_string()),
        }
    }
        ...
        match expr {
            ...
            Expression::BinaryOp { left, op, right } => {
                ...
                match op {
                    ...
                    BinaryOperator::Divide => Ok(format!("sdiv i32 {}, {}", left_code, right_code)),
                    BinaryOperator::Equal => Ok(format!("eq i32 {}, {}", left_code, right_code)),
                    BinaryOperator::NotEqual => Ok(format!("ne i32 {}, {}", left_code, right_code)),

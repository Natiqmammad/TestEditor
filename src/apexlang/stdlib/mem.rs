...
pub(super) fn register(registry: &mut NativeRegistry) {
    ...
    add!(functions, "smart_pointer_clone", smart_pointer_clone);
    add!(functions, "smart_pointer_free", smart_pointer_free);
...
fn smart_pointer_clone(args: &[Value]) -> Result<Value, ApexError> {
    let handle = expect_smart_handle(args, 0, "smart_pointer_clone")?;
    let mut table = SMART_POINTERS
        .lock()
        .map_err(|_| ApexError::new("Smart pointer table lock poisoned"))?;
    let value = table
        ...
        .cloned()
        .ok_or_else(|| ApexError::new("Unknown smart pointer handle"))?;
    let new_handle = table.next_handle;
    table.next_handle += 1;
    table.values.insert(new_handle, value);
    Ok(Value::Tuple(vec![Value::Int(BigInt::from(new_handle))]))
}

fn smart_pointer_free(args: &[Value]) -> Result<Value, ApexError> {
    let handle = expect_smart_handle(args, 0, "smart_pointer_free")?;
    let mut table = SMART_POINTERS
        .lock()
        .map_err(|_| ApexError::new("Smart pointer table lock poisoned"))?;
    Ok(Value::Bool(table.values.remove(&handle).is_some()))
}
...
mod tests {
    ...
    #[test]
    fn smart_pointer_clone_and_free() {
        let handle = smart_pointer_new(&[Value::Int(7.into())]).expect("create");
        let clone = smart_pointer_clone(&[handle.clone()]).expect("clone");
        let cloned_value = smart_pointer_get(&[clone.clone()]).expect("get clone");
        assert_eq!(cloned_value, Value::Int(7.into()));
        let freed = smart_pointer_free(&[handle.clone()]).expect("free");
        assert_eq!(freed, Value::Bool(true));
        let freed_again = smart_pointer_free(&[handle]).expect("free again");
        assert_eq!(freed_again, Value::Bool(false));
        let still_live = smart_pointer_get(&[clone]).expect("clone intact");
        assert_eq!(still_live, Value::Int(7.into()));
    }

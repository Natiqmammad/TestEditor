...
pub(super) fn register(registry: &mut NativeRegistry) {
    ...
    macro_rules! add {
        ($map:expr, $name:literal, $func:ident) => {
            $map.insert(
                $name.to_string(),
                NativeCallable::new(concat!("memory::", $name), $func),
            );
        };
    }
    ...
    registry.register_module("memory", functions);
}
...
fn tuple_get(args: &[Value]) -> Result<Value, ApexError> {
    let tuple = expect_tuple_arg(args, 0, "memory.tuple_get")?;
    let index = expect_usize_arg(args, 1, "memory.tuple_get")?;
    tuple
        .get(index)
        .cloned()

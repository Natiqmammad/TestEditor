...
use crate::codegen::{BytecodeGenerator, LLVMCodeGenerator, WASMCodeGenerator};
...
impl FlutterIntegration {
    ...
    fn generate_android_library(
        ...
        // Save to target path
        std::fs::write(&target.output_path.join("afns_native.c"), &output)
    ...
    fn generate_ios_framework(
        ...
        // Save files
        std::fs::write(&target.output_path.join("AfnsNative.h"), header_str)
            .map_err(|e| e.to_string())?;
        std::fs::write(&target.output_path.join("AfnsNative.m"), output)
            .map_err(|e| e.to_string())?;
    ...
    fn generate_web_module(
        ...
        std::fs::write(&target.output_path.join("afns.d.ts"), ts_definitions)
            .map_err(|e| e.to_string())?;
    ...
    fn generate_desktop_plugin(
        ...
        match target.arch.as_str() {
            "x86_64-pc-windows-msvc" => {
                ...
                std::fs::write(&target.output_path.join("afns.def"), def_content)
                    .map_err(|e| e.to_string())?;
            }
            "x86_64-apple-darwin" => {
        ...
        std::fs::write(&target.output_path.join("afns_desktop.c"), output)
            .map_err(|e| e.to_string())?;

        Ok(format!(
            "Desktop plugin generated for {}: {:?}",
            target.arch, target.output_path
        ))
    }

    /// Generate Flutter plugin manifest
    pub fn generate_flutter_plugin(
        &mut self,
        program: &Program,
        output_dir: PathBuf,
    ) -> Result<String, String> {
...
```"#;
        ...
        std::fs::write(&output_dir.join("pubspec.yaml"), pubspec_content)
            .map_err(|e| e.to_string())?;

        Ok(format!(
            "Flutter plugin manifest generated: {:?}",
            output_dir
        ))
    }
}

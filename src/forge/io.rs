...
impl AFNSFileIO {
    ...
    pub fn open_read(file_path: &str) -> Result<File, String> {
        File::open(file_path).map_err(|e| format!("Failed to open file for reading: {}", e))
    }

    /// Open file for writing
    pub fn open_write(file_path: &str) -> Result<File, String> {
        File::create(file_path).map_err(|e| format!("Failed to create file for writing: {}", e))
    }
...
impl AFNSSerialSerializer {
    ...
    pub fn to_yaml<T: serde::Serialize>(data: &T) -> Result<String, String> {
        serde_yaml::to_string(data).map_err(|e| format!("YAML serialization failed: {}", e))
    }

    /// Deserialize data from YAML
    pub fn from_yaml<T: serde::de::DeserializeOwned>(yaml: &str) -> Result<T, String> {
        serde_yaml::from_str(yaml).map_err(|e| format!("YAML deserialization failed: {}", e))
    }
}
